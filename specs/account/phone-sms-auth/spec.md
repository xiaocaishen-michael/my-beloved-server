...
>
> - `specs/account/register-by-phone/`（M1.1 落地）— 路径 `POST /api/v1/accounts/register-by-phone` 删除
> - `specs/account/login-by-phone-sms/`（M1.1 落地）— 路径 `POST /api/v1/auth/login-by-phone-sms` 删除
> - `specs/account/login-by-password/`（M1.1 落地）— 路径 `POST /api/v1/auth/login-by-password` 删除
>
> 上述三个目录加 `SUPERSEDED.md` 但保留原 spec 历史。
>
> **决策约束**：M1.2 docs-only；前端 spec 重写见 app 仓 [`apps/native/specs/login/`](https://github.com/xiaocaishen-michael/no-vain-years-app/blob/main/apps/native/specs/login/)；mockup 重做由 user 单独跑 Claude Design（per `docs/experience/claude-design-handoff.md` § 2.1b 合一页 prompt 模板）。
...
**落点**：本 spec FR-005（第 3 分支拆开 FROZEN/ANONYMIZED 单独表述）+ FR-006（timing defense 范围明示，FROZEN 不参与）+ SC-003（IT 路径数 4→3）；server 实现 + 同 PR 落地详见 [`specs/account/expose-frozen-account-status/`](../expose-frozen-account-status/)。
...
- 历史 spec：[`specs/account/register-by-phone/`](../register-by-phone/) / [`specs/account/login-by-phone-sms/`](../login-by-phone-sms/) / [`specs/account/login-by-password/`](../login-by-password/)（各加 SUPERSEDED.md）
