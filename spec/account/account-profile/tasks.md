...
-- per spec/account/account-profile/spec.md FR-007 / plan.md § V6 migration
...
**位置**：app 仓（独立 PR，不在本 spec 范围）。详见 [`apps/native/spec/onboarding/tasks.md`](https://github.com/xiaocaishen-michael/no-vain-years-app/blob/main/apps/native/spec/onboarding/tasks.md)（同 PR cycle 改写）。
...
- [`spec/account/phone-sms-auth/`](../phone-sms-auth/) — 上游 use case，响应 schema 不可变约束
