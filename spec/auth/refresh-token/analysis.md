...
> 本 use case 是 Phase 1 中**最复杂**的：引入新表 `account.refresh_token` + retrofit 3 既有 UseCase（register / 1.1 / 1.2）+ 跨 use case 横切关注（spec 组织在 `spec/auth/` 而非 `spec/account/`，但 module 仍 `mbw-account`）。本 analysis 重点核对**新引入决策**的端到端一致性 + retrofit 范围的明确性。
...
| 原则 | 检查 | 状态 |
|------|------|------|
| I. Modular Monolith | 工作仅在 `mbw-account`；spec 组织在 `spec/auth/` 仅作命名约定，不创建新模块 | ✅ |
...
**实施时机**：implement PR (PR 2) 内 T10 task 描述补充，不阻塞本 spec/plan/tasks PR。
